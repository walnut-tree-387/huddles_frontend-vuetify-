<template>
    <v-card class="mx-auto join-request-list">
        <v-card-title style="text-align: center font-size: large; font-weight: bolder; "> Join Requests </v-card-title>
        <v-divider></v-divider>
        <v-list shaped dense>
            <v-list-item-group>
                    ...
                    <v-list-item v-else :value="item" class="huddle-list-item">
                        <v-row align="center" class="w-100">
                            ...
                            <v-col cols="4" class="d-flex align-center">
                                <v-list-item-content>
                                <v-list-item-title v-text="item?.huddleMember?.name" class="truncate"></v-list-item-title>
                                </v-list-item-content>
                            </v-col>
                            <v-col cols="6" class="d-flex">
                                <v-list-item-action>
                                    <WalnutTreePrimaryAddButton :color="'green'" size="small" :title="'Accept'" @click="() => processRequest(item.huddleMember.uuid, 'ACCEPT')"/>
                                </v-list-item-action>
                                <v-list-item-action>
                                    <WalnutTreePrimaryRemoveButton :color="'red'" size="small" :title="'Reject'" @click="() => processRequest(item.huddleMember.uuid, 'DENIED')"/>
                                </v-list-item-action>
                            </v-col>
                        </v-row>
                    </v-list-item>
                ...
                <template v-else>
                    <span v-text="'No new join request..'" style="font-size: small; font-weight: lighter; text-align: center; display: block;"></span>
                </template>
            </v-list-item-group>
        </v-list>
    </v-card>
...
export default {
    components : {HuddleRemoveButton, WalnutUserAvatar, WalnutTreePrimaryAddButton, WalnutTreePrimaryRemoveButton},
    data() {
        return {
            items: []
        }
        ...
        async processRequest(userUuid: any, action: string) {
            ...
            try {
                ...
                if(status === 204){
                    ...
                    this.getJoinRequestList();
                    action === 'ACCEPT' ? this.showSuccess('Member added to huddle successfully') 
